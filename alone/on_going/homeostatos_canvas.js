...
function homeo() {
	...
	$("#poem").drawText({ 
		...
		fillStyle: "#36c", 
		fontSize: "12pt", 
		fontFamily: "DejaVu Sans Mono, Liberation Mono, Bitstream Vera Sans Mono, monospace", 
		text: fullexp,
		x: 20,
		y: 20,
		align: "left",
	...
	while (++counter < max_homeos) {
		for (var i = 0; i < effective_grammar.length; i++) {
			...
			for (var j = 0; j < effective_grammar[i].length; j++) {
				tmp_line += word_classes[effective_grammar[i][j]][get_random(word_classes[effective_grammar[i][j]].length)] + "\n";
			}
			homeostatos.push(tmp_line);
		}
	}
...
function estatos(homeostatos) {
	...
	for (var i = 0; i < homeostatos.length; i++) {
		sub_homeos = homeostatos[i].split("\n");
		for (var j = 0; j < sub_homeos.length; j++) {
			$("#poem").drawText({ 
				layer: true,
				name: "text" + (i + 1),
				fillStyle: "#36c", 
				...
				text: sub_homeos[j],
				x: 20,
				y: 20 + (i + j + additionals) * 20,
				align: "left",
	  			maxWidth: "100%",
	  			fromCenter: false
	    	});	
    	}
    	additionals += sub_homeos.length - 1;			
	}
   //$("#poem" + pnum).setLayer("text" + pnum + "." + String(lnum) + "." + String(block_counter), { text: line });
   $("#poem").drawLayers();
...
function init_canvas() {
	var ctx = $("#poem")[0].getContext("2d");
	//ctx.canvas.height = w_height;
	ctx.canvas.height = 12000;
